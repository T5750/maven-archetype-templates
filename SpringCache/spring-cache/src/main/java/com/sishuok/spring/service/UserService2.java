...
public class UserService2 {
	...
	@Caching(cacheable = { @Cacheable(value = "user", key = "#id") }, put = {
	...
	@Caching(cacheable = {
			@Cacheable(value = "user", key = "#username") }, put = {
					@CachePut(value = "user", key = "#result.id", condition = "#result != null"),
					@CachePut(value = "user", key = "#result.email", condition = "#result != null") })
	public User findByUsername(final String username) {
		System.out.println(
				"cache miss, invoke find by username, username:" + username);
		for (User user : users) {
	...
	@Caching(cacheable = { @Cacheable(value = "user", key = "#email") }, put = {
			@CachePut(value = "user", key = "#result.id", condition = "#result != null"),
			@CachePut(value = "user", key = "#result.email", condition = "#result != null") })
	...
	@Caching(evict = {
			// @CacheEvict(value = "user", key = "#user.id", condition =
			// "#root.target.canCache() and
			// #root.caches[0].get(#user.id).get().username ne #user.username",
