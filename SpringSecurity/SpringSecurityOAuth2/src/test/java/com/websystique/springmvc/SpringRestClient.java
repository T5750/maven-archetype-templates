...
public class SpringRestClient {
	...
	private static void listAllUsers(AuthTokenInfo tokenInfo) {
		...
		ResponseEntity<List> response = restTemplate.exchange(REST_SERVICE_URI
				+ "/user/" + QPM_ACCESS_TOKEN + tokenInfo.getAccess_token(),
	...
	private static void getUser(AuthTokenInfo tokenInfo) {
		...
		ResponseEntity<User> response = restTemplate.exchange(REST_SERVICE_URI
				+ "/user/1" + QPM_ACCESS_TOKEN + tokenInfo.getAccess_token(),
	...
	private static void createUser(AuthTokenInfo tokenInfo) {
		...
		URI uri = restTemplate.postForLocation(REST_SERVICE_URI + "/user/"
				+ QPM_ACCESS_TOKEN + tokenInfo.getAccess_token(), request,
				User.class);
	...
	private static void deleteUser(AuthTokenInfo tokenInfo) {
		...
		RestTemplate restTemplate = new RestTemplate();
		HttpEntity<String> request = new HttpEntity<String>(getHeaders());
		restTemplate.exchange(REST_SERVICE_URI + "/user/3" + QPM_ACCESS_TOKEN
				+ tokenInfo.getAccess_token(), HttpMethod.DELETE, request,
				User.class);
	}
	...
	private static void deleteAllUsers(AuthTokenInfo tokenInfo) {
		...
		RestTemplate restTemplate = new RestTemplate();
		HttpEntity<String> request = new HttpEntity<String>(getHeaders());
		restTemplate.exchange(REST_SERVICE_URI + "/user/" + QPM_ACCESS_TOKEN
				+ tokenInfo.getAccess_token(), HttpMethod.DELETE, request,
				User.class);
	}
