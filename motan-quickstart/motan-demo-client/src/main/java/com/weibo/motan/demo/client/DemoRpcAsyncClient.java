...
public class DemoRpcAsyncClient {
	 ...
	 * 2、在项目pom.xml中增加build-helper-maven-plugin，用来把自动生成类的目录设置为source path。
	 * 参见motan-demo-api模块的pom声明。
	...
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext(
				new String[] { "classpath:motan_demo_async_client.xml" });
		MotanDemoServiceAsync service = (MotanDemoServiceAsync) ctx
		...
		for (int i = 0; i < Integer.MAX_VALUE; i++) {
			...
			// multi call
			ResponseFuture future1 = service
					.helloAsync("motan async multi-1-" + i);
			ResponseFuture future2 = service
					.helloAsync("motan async multi-2-" + i);
			System.out.println(future1.getValue() + ", " + future2.getValue());
			// async with listener
			ResponseFuture future3 = service
					.helloAsync("motan async multi-1-" + i);
			ResponseFuture future4 = service
					.helloAsync("motan async multi-2-" + i);
