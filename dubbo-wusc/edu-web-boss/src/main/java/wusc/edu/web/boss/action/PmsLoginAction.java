...
public class PmsLoginAction extends BaseAction {
	...
	public String userLogin() {
		try {
			...
			if (user.getStatus().intValue() == UserStatusEnum.INACTIVE
					.getValue()) {
			...
			if (user.getUserPwd().equals(DigestUtils.sha1Hex(pwd))) {
				// 用户信息，包括登录信息和权限
				this.getSessionMap().put(SessionConstant.USER_SESSION_KEY,
						user);
				// 将主帐号ID放入Session
				if (UserTypeEnum.MAIN_USER.getValue()
						.equals(user.getUserType())) {
					this.getSessionMap().put(
							SessionConstant.MAIN_USER_ID_SESSION_KEY,
							user.getId());
				} else if (UserTypeEnum.SUB_USER.getValue()
						.equals(user.getUserType())) {
					this.getSessionMap().put(
							SessionConstant.MAIN_USER_ID_SESSION_KEY,
							user.getMainUserId());
					...
					// 其它类型用户的主帐号ID默认为0
					this.getSessionMap()
							.put(SessionConstant.MAIN_USER_ID_SESSION_KEY, 0L);
				}
				this.putData("userNo", userNo);
				...
				String msg = "";
				if (user.getPwdErrorCount()
						.intValue() >= SessionConstant.WEB_PWD_INPUT_ERROR_LIMIT) {
					// 超5次就冻结帐号
					user.setStatus(UserStatusEnum.INACTIVE.getValue());
					msg += "<br/>密码已连续输错【"
							+ SessionConstant.WEB_PWD_INPUT_ERROR_LIMIT
							+ "】次，帐号已被冻结";
				} else {
					msg += "<br/>密码错误，再输错【"
							+ (SessionConstant.WEB_PWD_INPUT_ERROR_LIMIT
