package wusc.edu.service.user.common.core.dao;
...
import wusc.edu.facade.user.common.exceptions.BizException;
...
public abstract class BaseDaoImpl<T extends BaseEntity>
	...
	public long insert(T entity) {
		...
		if (result <= 0) {
			throw BizException.DB_INSERT_RESULT_0.newInstance(
					"数据库操作,insert返回0.{%s}", getStatement(SQL_INSERT));
		}
	...
	public int update(T entity) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance(
					"数据库操作,update返回0.{%s}", getStatement(SQL_UPDATE));
		}
		return result;
	}
	...
	public int update(List<T> list) {
		...
		int result = sessionTemplate.update(getStatement(SQL_BATCH_UPDATE),
				list);
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance(
					"数据库操作,update返回0.{%s}", getStatement(SQL_UPDATE));
	...
	public PageBean listPage(PageParam pageParam,
		...
		if (isCount != null && "1".equals(isCount.toString())) {
			...
			return new PageBean(pageParam.getPageNum(),
					pageParam.getNumPerPage(), count.intValue(), list,
					countResultMap);
		} else {
			// 构造分页对象
			return new PageBean(pageParam.getPageNum(),
	...
	public T getBy(Map<String, Object> paramMap) {
		...
		return (T) sessionTemplate.selectOne(getStatement(SQL_LIST_BY),
				paramMap);
	}
