...
public class SysLogRepositoryJdbc implements SysLogRepository {
	...
	public void add(SysLog sysLog) {
		jdbcTemplate
				.update("INSERT syslog (uid,content,operation,createTime,user) VALUES (?,?,?,?,?)",
						sysLog.getUid(), sysLog.getContent(),
						sysLog.getOperation(), sysLog.getCreateTime(),
						sysLog.getUser());
	}

	@Override
	public List<SysLog> list() {
		return jdbcTemplate.query("select * from syslog",
