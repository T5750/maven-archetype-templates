...
public class MenuRepositoryJdbc implements MenuRepository {
	...
	public void add(Menu menu) {
		jdbcTemplate
				.update("INSERT menu (id,label,path,`level`,`order`,`url`,`type`,`style`,`disabled`) VALUES (?,?,?,?,?,?,?,?,?)",
						menu.getId(), menu.getLabel(), menu.getPath(), menu
								.getLevel(), menu.getOrder(), menu.getUrl(),
						menu.getType(), menu.getStyle(), menu.isDisabled() ? 1
								: 0);
	}
	...
	public void update(Menu menu) {
		jdbcTemplate
				.update("update menu SET label=?,`order`=?,url=?,disabled=?,`type`=?,`style`=? WHERE id=?",
						menu.getLabel(), menu.getOrder(), menu.getUrl(),
						menu.isDisabled() ? 1 : 0, menu.getType(),
						menu.getStyle(), menu.getId());
	}

	@Override
	public Menu get(String id) {
		return jdbcTemplate.queryForObject("select * from menu where id=?",
				createMapper(), id);
	}
	...
	public boolean contains(String id) {
		return jdbcTemplate.queryForObject(
				"select count(id) from menu where id=?", Integer.class, id) > 0;
	}
	...
	public void switchStatus(String id, boolean disabled) { // 禁用父节点没必要禁用所有子节点，同上
		jdbcTemplate.update("update menu SET disabled=? WHERE id=?",
				disabled ? 1 : 0, id);
	}
	...
	public List<Menu> roleMenus(String roleId) {
		return jdbcTemplate
				.query("select m.* from menu m join role_menu rm on m.id=rm.menu_id where rm.role_id=?",
						createMapper(), roleId);
	}
	...
	public List<Menu> getNavMenus(String userId) {
		return jdbcTemplate
				.query("select m.* from menu m join role_menu rm on m.id=rm.menu_id join user_role ur on rm.role_id=ur.role_id where m.disabled=0 and ur.uid=?",
						createMapper(), userId);
	}
}
