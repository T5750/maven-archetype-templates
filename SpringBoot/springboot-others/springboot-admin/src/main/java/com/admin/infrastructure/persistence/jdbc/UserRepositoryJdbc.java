...
public class UserRepositoryJdbc implements UserRepository {
	...
	public void add(User user) {
		jdbcTemplate
				.update("INSERT user (id,username,password,email,disabled,createTime,salt) VALUES (?,?,?,?,?,?,?)",
						user.getId(), user.getUsername(), user.getPassword(),
						user.getEmail(), user.isDisabled() ? 1 : 0, new Date(),
						user.getSalt());
	}
	...
	public boolean hasResourcePermission(String uid, String resourceCode) {
		return jdbcTemplate
				.query("select count(*) from user_role ur join role_resource rr on ur.role_id=rr.role_id where ur.uid=? and rr.resource_id=?",
						rs -> rs.getInt(0) > 0, uid, resourceCode);
	}
	...
	public void switchStatus(String id, boolean disabled) {
		jdbcTemplate.update("update user SET disabled=? WHERE id=?",
				disabled ? 1 : 0, id);
	}
