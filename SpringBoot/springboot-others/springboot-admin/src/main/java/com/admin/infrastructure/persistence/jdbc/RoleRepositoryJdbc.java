...
public class RoleRepositoryJdbc implements RoleRepository {
	...
	public void add(Role role) {
		jdbcTemplate
				.update("INSERT role (id,`name`,disabled,description) VALUES (?,?,?,?)",
						role.getId(), role.getName(),
						role.isDisabled() ? 1 : 0, role.getDescription());
	}

	@Override
	public void update(Role role) {
		jdbcTemplate
				.update("update role SET `name` =?,disabled=?,description=? WHERE id=?",
						role.getName(), role.isDisabled() ? 1 : 0,
	...
	public void updateMenus(String rid, List<String> mids) {
		...
		if (!CollectionUtils.isEmpty(mids)) {
			jdbcTemplate.batchUpdate(
					"INSERT role_menu (role_id,menu_id) VALUES (?,?)",
	...
	public boolean contains(String roleName) {
		return jdbcTemplate.queryForObject(
				"select count(`name`) from role where `name`=?", Integer.class,
				roleName) > 0;
	}

	@Override
	public Role get(String id) {
		return jdbcTemplate.queryForObject("select * from role where id=?",
	...
	public void removeRoleResourceByResourceId(String resourceId) {
		jdbcTemplate.update("DELETE FROM role_resource WHERE resource_id=?",
				resourceId);
	}

	public void switchStatus(String id, boolean disabled) {
		jdbcTemplate.update("update role SET disabled=? WHERE id=?",
				disabled ? 1 : 0, id);
	}
	...
	public List<Role> getRoles(String userId) {
		return jdbcTemplate
				.query("select * from role r join user_role ur on r.id=ur.role_id where ur.uid=?",
