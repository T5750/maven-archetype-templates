...
public interface UserRoleMapper {
	...
	@Select({
			"SELECT r.id, r.name, r.create_time, r.creator, r.description,u.name creatorUser,u1.userid ",
	...
	@Select({ "select", "userid, roleid, creator, create_time",
			"from user_role", "where userid = #{userid,jdbcType=INTEGER}",
			"and roleid = #{roleid,jdbcType=INTEGER}" })
	@Results({
			@Result(column = "userid", property = "userid", jdbcType = JdbcType.INTEGER, id = true),
			@Result(column = "roleid", property = "roleid", jdbcType = JdbcType.INTEGER, id = true),
			@Result(column = "creator", property = "creator", jdbcType = JdbcType.INTEGER),
			@Result(column = "create_time", property = "createTime", jdbcType = JdbcType.TIMESTAMP) })
	UserRole selectByPrimaryKey(@Param("userid") Integer userid,
								@Param("roleid") Integer roleid);
