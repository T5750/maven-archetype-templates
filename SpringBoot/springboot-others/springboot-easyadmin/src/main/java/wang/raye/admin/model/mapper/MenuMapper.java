...
public interface MenuMapper {
	...
	@Delete({ "delete from menu", "where id = #{id,jdbcType=INTEGER}" })
	...
	@Select({
			"SELECT m.id, m.name, m.url, m.icon, m.menu_type, m.display, m.parent_id, m.status,",
			"r.roleid FROM menu m LEFT JOIN (SELECT roleid,menuid FROM role_menu WHERE roleid=#{roleid})",
			" r ON m.id=r.menuid " })
	@Results({ @Result(column = "parent_id", property = "parentId", jdbcType = JdbcType.INTEGER) })
	 ...
	 */
	@Select({
			"SELECT id, name, url, icon, menu_type, display, parent_id FROM menu WHERE id IN(",
	 ...
	 */
	@Select({
			"SELECT url FROM menu WHERE id IN(",
			"SELECT menuid FROM role_menu WHERE roleid IN ",
	...
	int updateByExampleSelective(@Param("record") Menu record,
								 @Param("example") MenuCriteria example);
	...
	@Update({ "update menu", "set name = #{name,jdbcType=VARCHAR},",
