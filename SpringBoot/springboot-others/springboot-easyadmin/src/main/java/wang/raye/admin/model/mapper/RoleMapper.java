...
public interface RoleMapper {
	...
	@Select({
			"SELECT r.id, r.name, r.create_time, r.creator, r.description,u.creator relationUserid",
			" FROM role r ", "LEFT JOIN user_role u ON r.id=u.roleid" })
	...
	@Results({
			...
			@Result(column = "creator", property = "creator", jdbcType = JdbcType.INTEGER),
			@Result(column = "description", property = "description", jdbcType = JdbcType.VARCHAR) })
	List<Role> select(@Param("begin") int begin,
	...
	@Update({ "update role", "set name = #{name,jdbcType=VARCHAR},",
